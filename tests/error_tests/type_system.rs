...
// --- Type narrowing soundness error tests ---

/// Verifies that a non-diverging clause *before* a diverging type guard does not leave the
/// variable narrowed after the `if`. The `$flag` branch can reach the trailing statement without
/// ever evaluating the `instanceof` guard, so `$x` is still the `Cat|Dog` union and `$x->meow()`
/// must be rejected. Regression for a rule that wrongly kept the complement whenever only the last
/// clause diverged.
#[test]
fn test_error_narrowing_not_kept_when_earlier_clause_falls_through() {
    expect_error(
        r#"<?php
        class Cat { function meow(): string { return "m"; } }
        class Dog {}
        function f($x, $flag): string {
            if ($flag) {
                echo "a";
            } elseif (!($x instanceof Cat)) {
                return "not-cat";
            }
            return $x->meow();
        }
        ...
        f(new Dog(), true);
        "#,
        "single nullable object type",
    );
}

/// Verifies that when different clauses narrow different variables, every narrowed variable is
/// restored after the `if`, not just the first one. The non-diverging `$x` clause means the
/// trailing statement can run with `$y` still the `Cat|Dog` union, so `$y->meow()` must be
/// rejected. Regression for a single-slot restore that leaked the narrowing of later variables.
#[test]
fn test_error_narrowing_restores_all_narrowed_variables() {
    expect_error(
        r#"<?php
        class Cat { function meow(): string { return "m"; } }
        class Dog {}
