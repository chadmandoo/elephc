...
- [x] `--strict-php` flag — accept only PHP-compatible constructs: an AST audit rejects extension syntax (`ifdef`, `packed class`, `extern`, `ptr_cast<T>`, `buffer_new<T>`, typed locals, `ptr`/`buffer<T>` annotations, reserved `__elephc_*` names) across the main file, includes, and autoloaded files, while extension builtins (`ptr_*`, `zval_*`, `buffer_*`, `class_attribute_*`; declared via the registry's new `extension` flag) behave as nonexistent — `function_exists()` is `false`, calls are undefined functions with a disabled-extension hint, and user code may declare those names, matching the PHP interpreter. Compiler preludes are exempt (image/web now call `internal` `__elephc_ptr_*` aliases), `buffer_len`/`buffer_free` moved to the builtin registry, and `--define` conflicts with the flag
- [x] `--strict-php` coverage for `eval()` — strict binaries mark the magician bridge as strict (`__elephc_eval_set_strict_php` emitted at eval-context init), hiding the 21 extension eval builtins from dispatch and from `function_exists`/`is_callable` inside eval'd code (runtime fatal on call, like any unknown function, preserving PHP's execute-time semantics); the AOT literal-eval classifiers route extension syntax (`ifdef`, typed locals) to the bridge so nothing elephc-only is ever AOT-executed; user functions shadowing extension names stay callable from eval; a cross-registry parity gate pins magician's extension set against the compiler registry (minus static-only `zval_*`)
