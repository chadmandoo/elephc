...
- [x] Phase 50 — `Phar` / `PharData` OOP baseline: the checker now injects builtin `Phar` and `PharData` classes with PHP-facing format/compression/signature constants, constructors that store the archive path, `addFromString()`, `delete()`, `compressFiles()`, `decompressFiles()`, and ArrayAccess methods (`offsetGet`, `offsetSet`, `offsetExists`, `offsetUnset`) lowered as synthetic PHP bodies over the existing `phar://` `file_get_contents()` / `file_put_contents()` / `unlink()` runtime paths plus the elephc-phar compression bridge. This gives `$phar->addFromString("entry", "data")`, `$phar->delete("entry")`, native-PHAR `Phar::GZ` / `Phar::BZ2` / `Phar::NONE` compression control, `$phar["entry"]` read/write, `isset($phar["entry"])`, and `unset($phar["entry"])` coverage for native PHAR plus tar/ZIP containers without new target-specific assembly. Tests: `test_phar_oop_array_access_read_write`, `test_phar_oop_add_from_string_writes_entries`, `test_phar_oop_array_access_unset_deletes_entry`, `test_phar_oop_delete_method_removes_entries`, `test_phar_oop_compress_and_decompress_files`. Deferred: full iterator API, metadata/stub APIs, tar/ZIP compression controls, and private-key signing.
...
- [ ] **`phar://` advanced writes** — native PHAR writes now preserve and update
  ...
  Native PHAR compression-control writes are supported through
  `Phar::compressFiles()` / `decompressFiles()` for `Phar::GZ`, `Phar::BZ2`,
  and `Phar::NONE`. Deferred: tar/ZIP compression-control writes and
...
- [ ] **`Phar` / `PharData` OOP API** — a baseline constructor/constants,
  ...
  `decompressFiles()`, and ArrayAccess read/write/isset surface is implemented
  ...
  `PharFileInfo`, metadata/stub APIs, tar/ZIP compression controls, and
  OpenSSL/private-key signing.
