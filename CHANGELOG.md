...
## [Unreleased]
- Fixed a heap corruption when an array grown inside a loop is promoted to mixed-element storage (issue #452): with `$vals[] = 1; $vals[] = 2.0;` in a loop body, the earlier write site was typed and lowered against the pre-promotion element type, so from the second iteration it wrote an unboxed scalar into boxed-cell storage — reading that element produced garbage or a segfault. Loop bodies now pre-scan `$a[] =`, `$a[$i] =`, and `array_push($a, …)` sites and fix the array's element type to `mixed` up front (checker and EIR lowering), materializing the promotion once before the loop so every write boxes correctly. Opaque in-loop sources alongside a concrete sibling type also widen (without spuriously widening same-typed rebuild loops such as `MultipleIterator::detachIterator`). Also fixes the silent value corruption and per-conversion leak of the string/float flavour of the same pattern.
