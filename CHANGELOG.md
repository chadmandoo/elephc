...
## [Unreleased]
- Fixed `match` (and ternary) arms with heterogeneous result types (issue #488): object/array/string/int/float/`null` mixes now merge to a boxed `mixed` result and keep each arm's runtime type instead of coercing every arm into one scalar-biased unified type (fatal object-to-string casts, silent string/array coercions, or compile errors). Checker inference joins all arms — including assign→inferred-return shapes — and agrees with EIR lowering; `gettype()` on nullable-int merge temps no longer segfaults by unboxing an inline tagged scalar as a boxed Mixed cell. Same-representation merges (`int`/`bool`, indexed arrays with different element types) remain a documented PHP divergence.
