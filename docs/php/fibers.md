...
| Some callable arrays still need direct literals | `new Fiber(...)` accepts closures, first-class callables, descriptor-valued variables, runtime string callbacks, static-method callable arrays, callable-array literals such as `[$object, "method"]`, and simple invokable-object variables. Stored instance-method callable arrays such as `$cb = [$object, "method"]; new Fiber($cb);` are still rejected so the compiler does not accidentally bind the current `$object` variable instead of the receiver stored in `$cb[0]`. |
