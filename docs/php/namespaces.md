...
| `spl_classes()` | Returns the indexed array of SPL/core class and interface names shipped by the compiler today (currently 37 entries: SPL/core interfaces, throwable types, SPL exception classes, runtime-backed Phase 4 containers, and Phase 5 storage iterators). The list grows as later phases ship more SPL types |
