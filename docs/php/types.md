...
| `is_callable()` | `is_callable($val): bool`    | Returns true for closures, first-class callables, and string literals case-insensitively naming a known builtin or user function. Non-literal strings, `[$obj, "method"]` arrays, and `__invoke` objects are tracked as follow-ups. |
