...
- `class_implements()`, `class_parents()`, and `class_uses()` return an empty array for string names that are not compiled into the binary; PHP returns `false` for that unresolved-name case.
