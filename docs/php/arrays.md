...
> Callback arguments can be string literals, runtime string names for user functions, first-class callable values, anonymous functions, arrow functions, or variables holding captured closures. `array_map()` resolves runtime string callback variables through descriptor dispatch and stores mixed result elements when the selected callback return shape is only known at runtime.
