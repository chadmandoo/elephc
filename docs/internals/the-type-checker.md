...
`src/types/fibers.rs` owns the additional static checks for Fiber callbacks. `new Fiber(...)` accepts closures, known callable variables, first-class callables, runtime string callbacks, static-method callable arrays, simple callable-array literals such as `[$object, "method"]`, and simple invokable-object variables. Literal string callbacks are resolved to user-function, builtin, extern, or public static-method signatures when possible; dynamic string variables defer descriptor selection to codegen. Stored instance-method callable-array variables are still rejected until the constructor can bind the receiver from the array slot itself. The visible callback parameter count is capped at seven start arguments, and by-reference callback start parameters are rejected when the signature is statically known. Closure captures and receiver environments live in callable descriptor capture slots rather than in Fiber's visible start-argument area. Values moving through `start()`, `resume()`, `suspend()`, and `getReturn()` are typed as boxed `mixed`.
