...
> {
    ...
    while *pos < tokens.len() && !matches!(tokens[*pos].0, Token::RBrace | Token::Eof) {
        ...
        if tokens[*pos].0 == Token::Const {
            ...
            *pos += 1; // consume `const`
            // PHP 8.3 typed class constant: `const TYPE NAME = value`. An untyped const has its
            // name directly before `=`; anything else before `=` is a declared type — speculatively
            // parse and discard it (ClassConst carries no type, so byte-parity with PHP is preserved).
