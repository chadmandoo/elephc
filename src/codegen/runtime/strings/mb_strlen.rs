...
//!   result is returned in x0 / rax (matching the `__rt_crc32` string-helper ABI).
//! - Counts UTF-8 codepoints: every byte whose top two bits are NOT `10` is a
//!   codepoint-leading byte; continuation bytes (`10xxxxxx`) are skipped. This
//!   matches `mb_strlen($s)` under UTF-8 (the AIC/default encoding). The optional
//!   encoding argument is not supported by the lowering (UTF-8 is assumed).

use crate::codegen::{emit::Emitter, platform::Arch};

/// Emits `__rt_mb_strlen(str_ptr, str_len) -> count`, the UTF-8 codepoint counter.
pub fn emit_mb_strlen(emitter: &mut Emitter) {
    ...
    emitter.blank();
    emitter.comment("--- runtime: mb_strlen (UTF-8 codepoint count) ---");
    ...
    emitter.instruction("b.eq __rt_mb_strlen_skip"); // yes → not a codepoint start
    emitter.instruction("add x0, x0, #1"); // count a codepoint-leading byte
...
fn emit_mb_strlen_x86_64(emitter: &mut Emitter) {
    ...
    emitter.instruction("je __rt_mb_strlen_skip_x86"); // yes → not a codepoint start
    emitter.instruction("inc eax"); // count a codepoint-leading byte
