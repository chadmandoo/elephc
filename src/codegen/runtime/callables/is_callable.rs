...
fn emit_string_aarch64(emitter: &mut Emitter) {
    ...
    emitter.instruction("b.ge __rt_is_callable_string_false");                  // no callable string matched any known function
...
fn emit_array_aarch64(emitter: &mut Emitter) {
    ...
    emitter.instruction("b.ne __rt_is_callable_array_false");                   // non-Mixed indexed arrays cannot hold object/string callable pairs
    ...
    emitter.instruction("cmp x0, #6");                                          // receiver slot must contain an object
    emitter.instruction("b.ne __rt_is_callable_array_false");                   // reject non-object receiver slots
...
fn emit_assoc_aarch64(emitter: &mut Emitter) {
    ...
    emitter.instruction("cmp x3, #6");                                          // direct receiver entry must contain an object
    emitter.instruction("b.ne __rt_is_callable_assoc_false");                   // reject direct non-object receiver entries
    ...
    emitter.instruction("cmp x0, #6");                                          // boxed receiver must contain an object
    emitter.instruction("b.ne __rt_is_callable_assoc_false");                   // reject boxed non-object receiver entries
    ...
    emitter.instruction("ldr x0, [sp, #8]");                                    // pass saved receiver object to method-name lookup
    abi::emit_call_label(emitter, "__rt_is_callable_method_name");              // x1/x2 already hold method pointer and length from hash_get
...
fn emit_string_x86_64(emitter: &mut Emitter) {
    ...
    emitter.instruction("jge __rt_is_callable_string_false_x86_64");            // no callable string matched any known function
    emitter.instruction("mov r11, QWORD PTR [rbp - 24]");                       // reload user function table pointer
    emitter.instruction("mov r12, r10");                                        // copy index before scaling to 24-byte entries
    emitter.instruction("shl r10, 4");                                          // compute index * 16 as the first part of index * 24
    emitter.instruction("shl r12, 3");                                          // compute index * 8 as the second part of index * 24
    emitter.instruction("add r10, r12");                                        // combine scaled parts into index * 24
    ...
    emitter.instruction("mov r12, QWORD PTR [r11 + 16]");                       // load optional active-variant symbol pointer
    emitter.instruction("test r12, r12");                                       // ordinary functions use a null active-symbol pointer
    ...
    emitter.instruction("mov r12, QWORD PTR [r12]");                            // read active implementation pointer for this variant group
    emitter.instruction("test r12, r12");                                       // include variant is callable only when an implementation is active
...
fn emit_array_x86_64(emitter: &mut Emitter) {
    ...
    emitter.instruction("jne __rt_is_callable_array_false_x86_64");             // non-Mixed indexed arrays cannot hold object/string callable pairs
    emitter.instruction("mov rax, QWORD PTR [rdi + 24]");                       // load boxed Mixed receiver slot
    abi::emit_call_label(emitter, "__rt_mixed_unbox");                          // unwrap receiver slot to runtime tag and payload
    emitter.instruction("cmp rax, 6");                                          // receiver slot must contain an object
    emitter.instruction("jne __rt_is_callable_array_false_x86_64");             // reject non-object receiver slots
...
fn emit_assoc_x86_64(emitter: &mut Emitter) {
    ...
    emitter.instruction("cmp rcx, 6");                                          // direct receiver entry must contain an object
    emitter.instruction("jne __rt_is_callable_assoc_false_x86_64");             // reject direct non-object receiver entries
    ...
    abi::emit_call_label(emitter, "__rt_mixed_unbox");                          // unwrap receiver entry to runtime tag and payload
    emitter.instruction("cmp rax, 6");                                          // boxed receiver must contain an object
    emitter.instruction("jne __rt_is_callable_assoc_false_x86_64");             // reject boxed non-object receiver entries
    ...
    emitter.instruction("mov rdx, rsi");                                        // move direct method string length to third argument register
    emitter.instruction("mov rsi, rdi");                                        // move direct method string pointer to second argument register
    emitter.instruction("mov rdi, QWORD PTR [rbp - 16]");                       // reload receiver object pointer into first argument register
    abi::emit_call_label(emitter, "__rt_is_callable_method_name");              // check whether the direct string names a public receiver method
    ...
    emitter.instruction("mov rdi, QWORD PTR [rbp - 16]");                       // reload receiver object pointer into first argument register
    abi::emit_call_label(emitter, "__rt_is_callable_method_name");              // rdx already holds unboxed method string length
    emitter.instruction("jmp __rt_is_callable_assoc_done_x86_64");              // keep method lookup result and restore frame
