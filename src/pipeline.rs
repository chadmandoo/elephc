...
use crate::{
    ...
    arity_ext_prelude, explode_prelude, resolver, runtime_cache, source_map, stdlib_prelude,
    random_randomizer_prelude, strtr_prelude, substr_compare_prelude, tz_prelude, types,
...
pub(crate) fn compile(config: CliConfig) {
    ...
    // Inject the standard-library prelude (mb_* / array_* / http_build_query in elephc-PHP)
    // only when the program references one of those functions and does not declare its own,
    ...
    let ast = substr_compare_prelude::inject_if_used(ast);
    let ast = random_randomizer_prelude::inject_if_used(ast);
    let ast = explode_prelude::inject_if_used(ast);
    let ast = arity_ext_prelude::inject_if_used(ast);
