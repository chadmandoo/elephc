...
fn emit_intdiv_overflow_throw(ctx: &mut FunctionContext<'_>) {
    ...
    match ctx.emitter.target.arch {
        Arch::AArch64 => {
            ctx.emitter.instruction("mov x0, #40");                                // request Throwable payload storage for the ArithmeticError
        ...
        Arch::X86_64 => {
            ...
            ctx.emitter.instruction("mov rax, 40");                                  // request Throwable payload storage for the ArithmeticError
