...
impl<'m, 'f> LoweringContext<'m, 'f> {
    ...
    pub(crate) fn store_local(&mut self, name: &str, value: LoweredValue, php_type: PhpType, span: Option<Span>) -> LoweredValue {
            ...
            && !transfer_callable_source_to_store
        {
            ...
            // (upstream refinement). When the source is Mixed and the ref cell's
            // previous type is Int, the ref cell store narrows via __rt_mixed_cast_int,
            // consuming the Mixed box; the release_if_owned at the end frees the
            // original without a paired incref, which is correct for narrowing.
