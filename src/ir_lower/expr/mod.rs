...
fn array_literal_element_type_for_ir(
    ...
    match &item.kind {
        ...
        // `FunctionCall`, whose sig is looked up above) — the syntactic fallback
        // reduces it to `Int`, so an object-returning method (`[$this->mk()]`)
        // would be stored in an `Array(Int)` and int-cast at the element store
        // (a hard backend error). Store method-call elements as `Mixed`: an
        // adaptive-boxed array is run-clean for objects and every other return
        // type, and matches the `Array(Mixed)` covariance the call boundary
        // already accepts.
        ExprKind::MethodCall { .. }
        | ExprKind::NullsafeMethodCall { .. }
