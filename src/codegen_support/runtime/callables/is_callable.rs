...
fn emit_mixed_aarch64(emitter: &mut Emitter) {
    ...
    emitter.instruction("cmp x0, #10");                                         // is the mixed payload a callable descriptor (boxed closure / first-class callable)?
    emitter.instruction("b.eq __rt_is_callable_mixed_true");                    // callable descriptors are inherently callable
    ...
    emitter.instruction("b __rt_is_callable_mixed_done");                       // skip the descriptor-true case after object lookup

    emitter.label("__rt_is_callable_mixed_true");
    emitter.instruction("mov x0, #1");                                          // a boxed callable descriptor is directly callable
...
fn emit_mixed_x86_64(emitter: &mut Emitter) {
    ...
    emitter.instruction("cmp rax, 10");                                         // is the mixed payload a callable descriptor (boxed closure / first-class callable)?
    emitter.instruction("je __rt_is_callable_mixed_true_x86_64");               // callable descriptors are inherently callable
    ...
    emitter.instruction("jmp __rt_is_callable_mixed_done_x86_64");              // skip the descriptor-true case after object lookup

    emitter.label("__rt_is_callable_mixed_true_x86_64");
    emitter.instruction("mov eax, 1");                                          // a boxed callable descriptor is directly callable
